...
from plstm.config.plstm_2d_layer_fused import pLSTM2DLayerFusedConfig
from plstm.nnx.plstm_2d_layer_fused import pLSTM2DLayerFused
...
def test_plstm_2d_layer_fused_mixed_precision(
    batch_size,
    height,
    width,
    input_dim,
    ...
    """Test pLSTM2DLayerFused with mixed precision configurations.
    ...
    # Create model
    model = pLSTM2DLayerFused(config, rngs=nnx.Rngs(rng))
    ...
    for param_name, param in module_named_params(model):
        assert param.dtype == getattr(jnp, param_dtype), f"Wrong dtype in param {param_name}"
