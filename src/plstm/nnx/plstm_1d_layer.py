...
from .plstm_1d import pLSTM1D_jax
...
class pLSTM1DLayer(ResidualModule):
    ...
    def __init__(self, config: pLSTM1DLayerConfig, *, rngs: nnx.Rngs):
        nnx.Module.__init__(self)
        self.config = config

        if self.config.additional_convolution:
            self.conv = Convolution1DLayer(self.config.convolution, rngs=rngs)
        self.source = SourceLayer(self.config.source, rngs=rngs)
        self.transition = LongRangeTransitionLayer(self.config.transition, rngs=rngs)
        self.mark = MarkLayer(self.config.mark, rngs=rngs)
        self.direct = DirectLayer(self.config.direct, rngs=rngs)
        self.query = QueryLayer(self.config.query, rngs=rngs)
        if not self.config.tie_query_key:
            self.key = KeyLayer(self.config.key, rngs=rngs)
        self.value_ = ValueLayer(self.config.value, rngs=rngs)

        if self.config.mhnorm is not None:
            self.mhnorm = self.config.mhnorm.instantiate(NormInterface, rngs=rngs)

        if self.config.outprojection:
            self.outprojection = nnx.Linear(
                in_features=self.config.num_heads * self.config.JO * self.config.DV,
        ...
        if self.config.additional_passthrough:
            self.passthrough = PassthroughLayer(self.config.passthrough, rngs=rngs)
