...
class ZerosInit(InitInterface):
    ...
    def __init__(self, config: ZerosInitConfig):
        self.config = config
        self._initializer = nnx.initializers.zeros_init()
...
class OnesInit(InitInterface):
    ...
    def __init__(self, config: OnesInitConfig):
        self.config = config
        self._initializer = nnx.initializers.ones_init()
...
class ConstantInit(InitInterface):
    ...
    def __init__(self, config: ConstantInitConfig):
        self.config = config
        self._initializer = nnx.initializers.constant(self.config.value)
...
class DiagonalInit(InitInterface):
    ...
    def __call__(self, key: PRNGKey, shape: tuple[int, ...], dtype: jnp.dtype) -> jnp.ndarray:
        ...
        diag = jnp.tile(
            jnp.eye(min(in_size, out_size), dtype=dtype), (max(1, in_size // out_size), max(1, out_size // in_size))
        )
...
class LinspaceInit(InitInterface):
    ...
    def __call__(self, key: PRNGKey, shape: tuple[int, ...], dtype: jnp.dtype) -> jnp.ndarray:
        ...
        return jnp.tile(
            jnp.arange(self.config.low, self.config.high + step, step, dtype=dtype).reshape(
...
class NormalInit(InitInterface):
    ...
    def __init__(self, config: NormalInitConfig):
        self.config = config
        self._initializer = nnx.initializers.normal(stddev=self.config.stddev)
...
class TruncatedNormalInit(InitInterface):
    ...
    def __init__(self, config: TruncatedNormalInitConfig):
        self.config = config
        self._initializer = nnx.initializers.truncated_normal(
...
class WangInit(InitInterface):
    ...
    def __init__(self, config: WangInitConfig):
        ...
        self._initializer = nnx.initializers.variance_scaling(
            scale=scale**2,
            mode="fan_in",
            distribution="normal",
