...
class LongRangeTransitionLayer(nnx.Module):
    """Long range transition layer."""

    config: LongRangeTransitionLayerConfig

    def __init__(self, config: LongRangeTransitionLayerConfig, *, rngs: nnx.Rngs):
        ...
        if self.config.transition_dim > 1:
            inproj_bias_shape = (config.num_heads, config.transition_dim, config.transition_dim)
            self.inproj_bias = nnx.Param(
                config.inproj_bias_init.instantiate(InitInterface)(rngs.params(), inproj_bias_shape, dtype=param_dtype)
            )
            ...
            if self.config.weight:
                ...
                self.inproj_weight = nnx.Param(
                    config.inproj_weight_init.instantiate(InitInterface)(rngs.params(), weight_shape, dtype=param_dtype)
                )

        eigenvalues_bias_shape = [config.num_heads, config.transition_dim]
        self.eigenvalues_bias = nnx.Param(
            config.eigenvalue_bias_init.instantiate(InitInterface)(
                rngs.params(), eigenvalues_bias_shape, dtype=param_dtype
            )
        )
        ...
        if self.config.weight:
            ...
            self.eigenvalues_weight = nnx.Param(
                config.eigenvalue_weight_init.instantiate(InitInterface)(rngs.params(), weight_shape, dtype=param_dtype)
            )

        # Initialize weights according to config
        if self.config.transition_dim > 1 and not self.config.symmetric:
            outproj_bias_shape = [config.num_heads, config.transition_dim, config.transition_dim]
            self.outproj_bias = nnx.Param(
                config.outproj_bias_init.instantiate(InitInterface)(
                    rngs.params(), outproj_bias_shape, dtype=param_dtype
                )
            )
            ...
            if self.config.weight:
                ...
                self.outproj_weight = nnx.Param(
                    config.outproj_weight_init.instantiate(InitInterface)(
                        rngs.params(), weight_shape, dtype=param_dtype
                    )
                )
    ...
    def __call__(self, x):
        ...
        if self.config.weight:
            eigenvals = (
                ...
                + weight_einsum("nsbc,...sc->...nsb", self.eigenvalues_weight, x).reshape(
        ...
        if self.config.transition_dim > 1:
            if self.config.weight:
                in_mat = self._normalize(
                    ...
                    + weight_einsum("nsbcd,...sd->...nsbc", self.inproj_weight.astype(dtype), x).reshape(
                ...
                if self.config.symmetric:
                    ...
                    out_mat = self._normalize(
                        ...
                        + weight_einsum("nsbcd,...sd->...nsbc", self.outproj_weight.astype(dtype), x).reshape(
                            *x.shape[:-2], self.config.num_heads, self.config.transition_dim, self.config.transition_dim
                        )
                    )
            else:
