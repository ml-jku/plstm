...
@dataclass
class pLSTM2DLayerConfig(ResidualModuleConfig):
    _shortname: str = "pLSTM2D"
    mode: Literal["D", "P"] = "P"
    ...
    orientation_combinations: list[int] = field(default_factory=lambda: [0, 1, 2, 3])
    ...
    def __post_init__(self):
        ...
        if self.source is None:
            self.source = SourceLayerConfig(
                num_heads=len(self.orientation_combinations) * self.num_heads,
                JK=self.JK,
                JV=self.JV,
                JT=self.JT,
            ...
            self.source.num_heads = len(self.orientation_combinations) * self.num_heads
        ...
        if self.transition is None:
            ...
            self.transition.num_heads = len(self.orientation_combinations) * self.num_heads
        ...
        if self.mark is None:
            ...
            self.mark.num_heads = len(self.orientation_combinations) * self.num_heads
